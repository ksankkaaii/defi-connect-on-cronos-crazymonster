...
import nftImage from '../Resource/images/Nft.jpg'
...
const Mintpage = () => {
  ...
  const updatePrice = async () => {
    ...
    const _newPublicPrice = Number(ethers.utils.formatEther(await nftContract.cost()))
    setPublicPrice(_newPublicPrice)

    const _newWhitelistPrice = Number(ethers.utils.formatEther(await nftContract.wlCost()))
    setWhitelistPrice(_newWhitelistPrice)
    ...
    if (isWhitelisted) {
      ...
      if (Number(balance) < 25) {
        setMintPrice(_newWhitelistPrice)
        return
      }
    }
    setMintPrice(_newPublicPrice)
  }
  ...
  return (
    <div className='pt-32'>
      <div className="flex flex-col items-center justify-center gap-12">
        {/* nft */}
        <div className="">
          <img src={nftImage} alt="" className="w-72 md:w-96 shadow-xl rounded-xl " />
        </div>
        {/* Details */}
